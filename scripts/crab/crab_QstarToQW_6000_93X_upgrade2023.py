...
inputDatasets = ['/QstarToQW_M_6000_TuneCUETP8M1_14TeV_pythia8/PhaseIITDRFall17MiniAOD-noPU_93X_upgrade2023_realistic_v2-v1/MINIAODSIM']
inputDatasets = autocomplete_Datasets(inputDatasets)
requestNames = ['QstarToQW_M_6000_14TeV_PhaseIITDRFall17_noPU']
for x in inputDatasets:
    name = x.split('/')[1]
    modified_name =name.replace('QstarToQW_M_6000_14TeV_noPU','')
    if 'ext' in x:
        modified_name += '_ext'
...
config = config()
config.General.workArea = 'crab_Qstar'
